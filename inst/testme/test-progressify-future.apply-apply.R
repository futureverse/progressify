...
## future_apply() calls FUN prod(dim(X)[MARGIN]) times, so the number of
## progress updates depends on MARGIN.
cases <- list(
  list(expr = quote(future_apply(m, 1, g)),       n = 2L),  # nrow
  list(expr = quote(future_apply(m, 2, g)),       n = 3L),  # ncol
  list(expr = quote(future_apply(m, c(1, 2), g)), n = 6L),  # per cell
  list(expr = quote(future_apply(m, "cols", g)),  n = 3L),  # dimname MARGIN
  list(expr = quote(future.apply::future_apply(X = m, MARGIN = 1, FUN = g)), n = 2L)
)
